...
$PHPTHUMB_CONFIG['cache_maxsize'] = 1000 * 1024 * 1024; // delete least-recently-accessed cached thumbnails when more than [1000MB] of cached files are present (value is maximum bytesize of all cached files)
...
$PHPTHUMB_CONFIG['high_security_password']   = 'ssa*&SJjh1Sj75os12s7SU18sjf76^25S651su6124';     // required if 'high_security_enabled' is true, and must be at complex (uppercase, lowercase, numbers, punctuation, etc -- punctuation is strongest, lowercase is weakest; see PasswordStrength() in phpThumb.php). You can use a password generator like http://silisoftware.com/tools/password-random.php to generate a strong password
...
function phpThumbURL($ParameterString) {
	...
	return str_replace('\\', '/', str_replace(@$PHPTHUMB_CONFIG['document_root'], '', dirname(__FILE__)).DIRECTORY_SEPARATOR.'phpThumb.php?'.$ParameterString.'&hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']));
}
