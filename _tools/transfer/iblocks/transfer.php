...
define('LOG_FILENAME', 'transfer.log');
...
if ($_REQUEST['work_start'] && check_bitrix_sessid()) {
    $rsEl = CIBlockElement::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]), false, array("nTopCount" => $limit), ['ID', 'IBLOCK_ID', 'NAME']);
    $El = new CIBlockElement();
    while ($obEl = $rsEl->GetNextElement()) {
        $arEl = $obEl->GetFields();
        $arEl['PROPERTIES'] = $obEl->GetProperties();
        $arFields['PROPERTY_VALUES'] = array_reduce(
            $arEl['PROPERTIES'],
            function ($carry, $item) {
                $carry[$item['CODE']] = $item['VALUE'];
                return $carry;
            },
            []
        );
        if(!$arFields['PROPERTY_VALUES']['FIXED_PRODUCT_NAME'])
            $arFields['PROPERTY_VALUES']['FIXED_PRODUCT_NAME']=$arEl['NAME'];

        //file_put_contents('1.txt',print_r($arFields,true),FILE_APPEND);
        $El->Update($arEl['ID'], $arFields, false, false, false, false);
