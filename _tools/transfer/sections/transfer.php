...
$arBaseFilter= array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]);

$arCustomFilter=array();

$arFilter=array_merge($arBaseFilter,$arCustomFilter);

if($_REQUEST['work_start'] && check_bitrix_sessid())
{
	$rsEl = CIBlockSection::GetList(array("ID" => "ASC"),$arFilter, false, array("nTopCount" => $limit));
	...
	$rsLeftBorder = CIBlockSection::GetList(array("ID" => "ASC"), $arFilter);
	$leftBorderCnt = $rsLeftBorder->SelectedRowsCount();

	$rsAll = CIBlockSection::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID));
