...
<div class="product_detail" id="product_detail">
    <div class="product_main_part">
        <div class="offers">
            ...
            <div class="offer current" <?if($arResult['_OFFERS_VIEW']):?><?if($arResult['_OFFERS_VIEW'] == 2):?><?foreach($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['PARAMS']['VALUE'] as $k=>$v):?>data-<?=Cutil::translit(trim($v),'ru');?>="<?=Cutil::translit(trim($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['PARAMS']['DESCRIPTION'][$k]),'ru');?>" <?endforeach;?><?else:?>data-offer="<?=$arResult['_CURRENT_OFFER']['ID']?>"<?endif;?><?endif;?><?if($arResult['_CURRENT_OFFER']['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']):?> data-ajax-url="<?=$arResult['_CURRENT_OFFER']['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']?>"<?endif;?>>
                <a href="/_tools/add2cart.php?id=<?=$arResult['_CURRENT_OFFER']['ID'];?>&backurl=<?if($arResult['_CURRENT_OFFER']['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']):?><?=urlencode($arResult['_CURRENT_OFFER']['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']);?><?else:?><?=urlencode($arResult['_CURRENT_OFFER']['DETAIL_PAGE_URL']);?><?endif;?><?//=$arResult['_CURRENT_OFFER']['BUY_URL']?>" class="buy"></a>
                <div class="price">
                    <div class="old_price">
                ...
                <div class="offer_params">
                    <?if($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['CODE']['VALUE'] or $arResult['DISPLAY_PROPERTIES']['CODE']['VALUE']):?>Артикул: <?if($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['CODE']['VALUE']):?><?=$arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['CODE']['VALUE']?><?else:?><?=$arResult['DISPLAY_PROPERTIES']['CODE']['VALUE']?><?endif;?>&nbsp;&nbsp;&nbsp;&nbsp; <?endif;?><span><?=$arResult['_CURRENT_OFFER']['NAME']?></span>
                </div>
            </div>
			<?endif;?>
            <?if(count($arResult['OFFERS'])>1):?>
            	<?foreach($arResult['OFFERS'] as $arOffer):?>
                	<?if($arOffer['CAN_BUY'] and ($arOffer['ID']!=$arResult['_CURRENT_OFFER']['ID'])):?>
                        <div class="offer" <?if($arResult['_OFFERS_VIEW'] == 2):?><?foreach($arOffer['DISPLAY_PROPERTIES']['PARAMS']['VALUE'] as $k=>$v):?>data-<?=Cutil::translit(trim($v),'ru');?>="<?=Cutil::translit(trim($arOffer['DISPLAY_PROPERTIES']['PARAMS']['DESCRIPTION'][$k]),'ru');?>" <?endforeach;?><?else:?>data-offer="<?=$arOffer['ID']?>"<?endif;?><?if($arOffer['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']):?> data-ajax-url="<?=$arOffer['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']?>"<?endif;?>>
                            <a href="/_tools/add2cart.php?id=<?=$arOffer['ID'];?>&backurl=<?=urlencode($arOffer['_IBLOCK_FIELDS']['DETAIL_PAGE_URL']);?><?//=$arOffer['BUY_URL']?>" class="buy"></a>
                            <div class="price">
                                <div class="old_price">
                            ...
                            <div class="clear"></div>
                            <div class="offer_params">
                                <?if($arOffer['DISPLAY_PROPERTIES']['CODE']['VALUE'] or $arResult['DISPLAY_PROPERTIES']['CODE']['VALUE']):?>Артикул: <?if($arOffer['DISPLAY_PROPERTIES']['CODE']['VALUE']):?><?=$arOffer['DISPLAY_PROPERTIES']['CODE']['VALUE']?><?else:?><?=$arResult['DISPLAY_PROPERTIES']['CODE']['VALUE']?><?endif;?>&nbsp;&nbsp;&nbsp;&nbsp; <?endif;?><span><?=$arOffer['NAME']?></span>
                            </div>
                        </div>
                    <?endif;?>
            	<?endforeach;?>
        ...
        <div class="offer_selector">
            ...
            <?endif;?>
            <div class="title"><?if($arResult['_OFFERS_VIEW']==1):?>Вариант<?else:?>Параметры<?endif;?>:</div>
            <div class="clear"></div>
                ...
                <?foreach($arResult['_SELECT_PARAMS'] as $k=>$arValues):?>
                    <select class="custom" data-param="<?=Cutil::translit($k,'ru');?>">
                        <?foreach($arValues as $v):?>
                            <option value="<?=Cutil::translit($v,'ru');?>"<?foreach($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['PARAMS']['VALUE'] as $m=>$n):?><?if(($n==$k) and ($arResult['_CURRENT_OFFER']['DISPLAY_PROPERTIES']['PARAMS']['DESCRIPTION'][$m]==$v)):?> selected<?endif;?><?endforeach;?>><?=$v?></option>
                        <?endforeach;?>
                    </select>
                    <div></div>
                <?endforeach;?>
            <?else:?>
                <select class="custom" data-param="offer">
                    <?foreach($arResult['OFFERS'] as $arOffer):?>
                        <option value="<?=$arOffer['ID']?>"<?if($arOffer['ID'] == $arResult['_CURRENT_OFFER']['ID']):?> selected<?endif;?>><?if($arResult['_OFFERS_VIEW'] == 1):?><?=$arOffer['NAME']?><?else:?><?foreach($arOffer['DISPLAY_PROPERTIES']['PARAMS']['VALUE'] as $m=>$n):?><?if($m>0):?>, <?endif;?><?=$arOffer['DISPLAY_PROPERTIES']['PARAMS']['DESCRIPTION'][$m]?><?endforeach;?><?endif;?></option>
                    <?endforeach;?>
                </select>
            <?endif;?>
        </div>
    ...
    <div class="product_gallery_part">
    	<?if(count($arResult['DISPLAY_PROPERTIES']['PICTURES']['FILE_VALUE'])):?>
            ...
            <div class="gallery_thumbs">
                <?foreach($arResult['DISPLAY_PROPERTIES']['PICTURES']['FILE_VALUE'] as $k=>$arFile):?>
                    <a href="" data-slide-index="<?=$k?>" style="background-image:url(<?=phpThumbURL('src=' . _PHPTHUMB_PATH_PREFIX . $arFile['SRC'] . '&w=70&h=70&zc=1&f=jpg&q=85');?>)"></a>
                <?endforeach;?>
            </div>
            <?endif;?>
            <div class="gallery_wrapper <?if(count($arResult['DISPLAY_PROPERTIES']['PICTURES']['FILE_VALUE'])==1):?>gallery_wrapper_no_thumbs<?endif;?>">
                ...
                <div class="product_gallery">
                    ...
                    ?>
                    <div>
                        <div class="image<?if($arResult['PROPERTIES']['NOT_CROP_IMG_'.$counter]['VALUE'] or $arResult['PROPERTIES']['NOT_CROP_ALL_IMAGES']['VALUE']):?> not_crop_img<?endif;?>" style="background-image:url(<?if($arFile['WIDTH']>1000):?><?=phpThumbURL('src=' . _PHPTHUMB_PATH_PREFIX . $arFile['SRC'] . '&w=1000&h=9999f=jpg&q=85');?><?else:?><?=$arFile['SRC']?><?endif;?>)">
                            <div class="zoomable">
                                <img src="<?=$arFile['SRC']?>" />
                            </div>
                        </div>
        ...
        <?elseif(count($arResult['DISPLAY_PROPERTIES']['PICTURES_LINKS']['VALUE'])):?>
            ...
            <div class="gallery_thumbs">
                <?foreach($arResult['DISPLAY_PROPERTIES']['PICTURES_LINKS']['VALUE'] as $k=>$v):?>
                    <a href="" data-slide-index="<?=$k?>" style="background-image:url(<?=phpThumbURL('src=' . $v . '&w=70&h=70&zc=1&f=jpg&q=85');?>)"></a>
                <?endforeach;?>
            </div>
            <?endif;?>
            <div class="gallery_wrapper <?if(count($arResult['DISPLAY_PROPERTIES']['PICTURES_LINKS']['VALUE'])==1):?>gallery_wrapper_no_thumbs<?endif;?>">
                <?if(strlen($arResult['_LABEL'])):?>
                    <div class="label"><?=$arResult['_LABEL']?></div>
                <?endif;?>
                <div class="product_gallery">
                    ...
                    ?>
                    <div>
                        <div class="image<?if($arResult['PROPERTIES']['NOT_CROP_IMG_'.$counter]['VALUE'] or $arResult['PROPERTIES']['NOT_CROP_ALL_IMAGES']['VALUE']):?> not_crop_img<?endif;?>" style="background-image:url(<?=$v?>)">
                            <div class="zoomable">
                                <img src="<?=$v?>" />
                            </div>
                        </div>
        ...
        <?else:?>
        	<div class="gallery_wrapper gallery_wrapper_no_thumbs">
                ...
                <div class="product_gallery">
                    <div>
                        <div class="image<?if($arResult['PROPERTIES']['NOT_CROP_PREVIEW_IMG']['VALUE'] or $arResult['PROPERTIES']['NOT_CROP_ALL_IMAGES']['VALUE']):?> not_crop_img<?endif;?>" style="background-image:url(<?=$arResult['PREVIEW_PICTURE']['SRC']?>)">
                            <div class="zoomable">
    ...
    <div class="tabbed_content">
        <ul class="tabs">
            ...
            <li><a href="#tab_specs"><img src="<?=$templateFolder?>/img/icon_chars.png" /><span>Характеристики</span></a></li>
            <?endif;?>
            <?if(strlen($arResult['DETAIL_TEXT'])):?>
            	<li><a href="#tab_description"><img src="<?=$templateFolder?>/img/icon_descr.png" /><span>Описание</span><div></div></a></li>
            <?endif;?>
            ...
            <li><a href="#tab_add_photos"><img src="<?=$templateFolder?>/img/icon_photos.png" /><span>Фотографии</span></a></li>
            <?endif;?>
            <?if(intval($arResult['DISPLAY_PROPERTIES']['AUTO_CALCULATED_REVIEWS_AMOUNT']['VALUE'])):?>
            	<li><a href="#tab_reviews"><img src="<?=$templateFolder?>/img/icon_comments.png" /><span>Отзывы</span></a></li>
            <?endif;?>
            <?if(count($arResult['_ALSO_BUY'])):?>
            <li><a href="#tab_seealso"><img src="<?=$templateFolder?>/img/icon_alsobuy.png" /><span>С этим товаром покупают</span></a></li>
            <?endif;?>
        </ul>
        <div class="inner">
            <?if(count($arResult['DISPLAY_PROPERTIES']['CHARS']['VALUE'])):?>
            ...
            <div id="tab_add_photos">
                ...
                <?foreach($arResult['DISPLAY_PROPERTIES']['ADD_PICTURES']['FILE_VALUE'] as $arFile):?>
                	<a href="<?=$arFile['SRC'] ?>" target="_blank"><img src="<?=phpThumbURL('src=' . _PHPTHUMB_PATH_PREFIX . $arFile['SRC'] . '&w=1000&h=500&f=jpg&q=85');?>" /></a>
